	...
	public static <T> T $Page(Class<T> oType) {
		...
		try {
			try {
				...
				String sPlatform = capabilities.getPlatform().toString();
				
				//String sPlatform = Driver.getDriverProperty().getPlatform();
				String sPageClass = String.format("%s.%s_%s", oType.getPackage().getName(), oType.getSimpleName(),
						sPlatform.toUpperCase());
